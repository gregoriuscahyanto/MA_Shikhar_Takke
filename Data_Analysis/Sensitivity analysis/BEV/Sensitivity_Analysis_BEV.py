...
SWEEP_CONFIG = {
    # fast real cars: failed simulations are usually too slow -> add EV torque/power and launch gearing
    ...
    # slow real cars: failed simulations are often too fast -> reduce EV power, add mass, slightly reduce gearing
...
def result_file_for_group(work_dir, group):
    ...
    candidates = [
        ...
        f"sweep_results_ev_{group}.xlsx",
        f"sweep_results_ev_{short}.xlsx",
        ...
        f"sweep_results_ev_{group}.csv",
        f"sweep_results_ev_{short}.csv",
...
def split_inputs(args):
    ...
    if input_file is None:
        raise FileNotFoundError("BEV/EV input CSV not found. Pass --input_csv.")
    ...
    if "EV" in df.columns:
        df = df[pd.to_numeric(df["EV"], errors="coerce").fillna(0).astype(int) == 1].copy()
    if "Hy" in df.columns:
        df = df[pd.to_numeric(df["Hy"], errors="coerce").fillna(0).astype(int) == 0].copy()
...
def generate_sweep_inputs(args):
    ...
    for g in GROUP_LABELS:
        ...
        new_run_id = 1
        cfg = SWEEP_CONFIG[g]
        keys = ["ev_power_scale", "gear_scale", "mass_scale", "bat_dis_scale"]

        for _, row in fails.iterrows():
            for ev_scale, gear_scale, ms, bs in itertools.product(*(cfg[k] for k in keys)):
                new_row = row[all_input_cols].copy()
                scale_existing(new_row, row, EV_SCALE_COLS, ev_scale)
                scale_existing(new_row, row, GEAR_SCALE_COLS, gear_scale, lower=0.05, upper=20.0)
                ...
                new_row["ev_power_scale"] = ev_scale
                new_row["gear_scale"] = gear_scale
...
def analyze(args):
    ...
    for g in sorted(df_all["group"].unique().tolist(), key=GROUP_LABELS.index):
        ...
        print(f"{g} best: EV power x{best['ev_power_scale']:.2f}, gear x{best['gear_scale']:.2f}, mass x{best['mass_scale']:.2f}, battery x{best['bat_dis_scale']:.2f} | pass {best['pass_rate']*100:.1f}%")
    ...
    ax.set_title("BEV sensitivity: original pass rate + recovered runs")
    ax.set_ylim(0, 115)
    ax.grid(True, alpha=0.3, axis="y")
    ax.legend(fontsize=8)
    plt.tight_layout()
    plt.savefig(output_dir / "plotA_bev_pass_rate_recovery.png", dpi=150)
    plt.close()
    ...
    for g in groups:
        ...
        for ax, data, xcol, ycol, title, flag in panels:
            ...
            ax.legend(fontsize=8)
        fig.suptitle(f"BEV {g}: actual vs simulated")
        plt.tight_layout()
        plt.savefig(output_dir / f"plotB_bev_scatter_{g}.png", dpi=150)
        plt.close()

    # Plot C + Excel export: recovered rows and mean parameter change
    export_path = output_dir / "bev_recovered_runs.xlsx"
    with pd.ExcelWriter(export_path, engine="openpyxl") as writer:
        ...
        for g in groups:
            ...
            if changes:
                ...
                ax.set_title(f"BEV {g}: mean parameter change in recovered runs")
                ax.grid(True, alpha=0.3, axis="x")
                plt.tight_layout()
                plt.savefig(output_dir / f"plotC_bev_mean_param_change_{g}.png", dpi=150)
                plt.close()
    print(f"Saved analysis outputs to {output_dir}")


def main():
    parser = argparse.ArgumentParser(description="BEV sensitivity analysis: split -> sweep -> analyze.")
    sub = parser.add_subparsers(dest="cmd")

    p_split = sub.add_parser("split", help="Create sensitivity_bev_G*.csv from original inputs/results.")
    ...
    p_sweep = sub.add_parser("sweep", help="Create sweep_bev_G*.csv from failed rows.")
    ...
    p_an.add_argument("--output_dir", default="bev_sensitivity_results")
    ...
    if args.cmd == "split":
        ...
        parser.print_help()
        print("\nTypical order:")
        print("  python Sensitivity_Analysis_BEV.py split --work_dir .")
        print("  python Sensitivity_Analysis_BEV.py sweep --work_dir .")
        print("  # run Simulink/SL simulations for sweep_bev_G*.csv and save sweep_results_bev_G*.xlsx")
        print("  python Sensitivity_Analysis_BEV.py analyze --work_dir . --sweep_results_dir .")
