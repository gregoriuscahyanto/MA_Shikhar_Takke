...
%   DoE_main_sensitivity('C:/.../sweep_bev_G1_lt5s.csv', 'C:/.../sweep_results_bev_G1.xlsx')
    ...
    % Read the CSV immediately. On HPC the pending resume CSVs live in a
    % plan directory that may be regenerated by task 0. Loading the config
    % before the slower Simulink initialization avoids a race where the file
    % exists at startup but is replaced before readtable() is reached.
